...
public class HystrixAtomicLongArrayNoHotReadsCachedSnapshotsRollingNumber extends HystrixRollingNumber<HystrixAtomicLongBucket> {
    @SuppressWarnings("unused")
    private static final Logger logger = LoggerFactory.getLogger(HystrixRollingNumber.class);

    private volatile long[] cachedBucketValues = new long[HystrixRollingNumberEvent.values().length]; //indexed by eventType
     ...
     */
    @Deprecated
    public HystrixAtomicLongArrayNoHotReadsCachedSnapshotsRollingNumber (HystrixProperty<Integer> timeInMilliseconds, HystrixProperty<Integer> numberOfBuckets) {
        this(timeInMilliseconds.get(), numberOfBuckets.get());
    }

    public HystrixAtomicLongArrayNoHotReadsCachedSnapshotsRollingNumber (int timeInMilliseconds, int numberOfBuckets) {
    ...
    public HystrixAtomicLongArrayNoHotReadsCachedSnapshotsRollingNumber (HystrixTime time, int timeInMilliseconds, int numberOfBuckets) {
        super(time, timeInMilliseconds, numberOfBuckets);

        for (int i = 0; i < HystrixRollingNumberEvent.values().length; i++) {
            cachedBucketValues[i] = 0L;
        }
    }
    ...
    protected long getWindowRollingSum(HystrixRollingNumberEvent type) {
        return cachedBucketValues[type.ordinal()];
    }
    ...
    protected void rollInternalDataStructures(BucketCircularArray buckets, HystrixAtomicLongBucket lastFullBucket) {
        long[] rollingSnapshot = new long[HystrixRollingNumberEvent.values().length];

        for (HystrixRollingNumberEvent eventType: HystrixRollingNumberEvent.values()) {
            rollingSnapshot[eventType.ordinal()] = 0;
        }

        for (HystrixRollingNumberEvent eventType: HystrixRollingNumberEvent.values()) {
            final int i = eventType.ordinal();
            long currentMax = 0;
            for (HystrixAtomicLongBucket bucket: buckets) {
                if (eventType.isCounter()) {
                    rollingSnapshot[i] += bucket.get(eventType);
                } else {
                    if (bucket.get(eventType) > currentMax) {
                        currentMax = bucket.get(eventType);
                    }
                }
            }
            if (eventType.isMaxUpdater()) {
                rollingSnapshot[i] = currentMax;
            }
        }
        cachedBucketValues = rollingSnapshot;
    ...
    public void reset() {
        ...
        cachedBucketValues = new long[HystrixRollingNumberEvent.values().length];
    }
